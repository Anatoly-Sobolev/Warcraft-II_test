...
- Действия игрока идут через Input и `WarcraftCommand`, а не из кнопки прямо в
  Warcraft Runtime.
- Оригинальные ассеты Warcraft II используются только как временные
  `original_placeholder` с записью в `content/assets/placeholder_manifest.json`
  и планом замены на новые ассеты.
...
Эти материалы можно использовать как runtime-ассеты только временно: добавить
entries в `content/assets/placeholder_manifest.json` и пометить файлы как
`original_placeholder`, пока дизайнер не заменит их новыми ассетами.
...
Для reference-only пакета можно хранить manifest: названия кадров, сценарий
получения, размер, дата снятия и внешний путь/носитель. Runtime-заглушки для
проекта кладутся отдельно в `content/assets/` и описываются в asset manifest.
...
Wargus можно использовать как справочник по структуре и mapping-данным. Реализация
в Godot оформляется в собственных runtime, catalog и presentation слоях проекта.
...
Если в этой папке лежит `original_placeholder`, дизайнер видит его как временную
форму и источник размеров/states, но задача остается прежней: заменить его новым
визуальным стилем поверх состава оригинального UI, а не просто перекрасить
placeholder-атласы.
...
warcraft-ii/
  ...
  content/
    ...
    assets/                    разрешенные ассеты, placeholders и manifest
```
...
- для новых ассетов понятно, что они разрешены к коммиту или являются
  placeholders;
- если используется `original_placeholder`, он имеет запись в manifest,
  replacement target и owner.
