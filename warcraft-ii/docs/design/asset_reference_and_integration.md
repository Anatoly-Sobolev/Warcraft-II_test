...
Оригинальные ассеты Warcraft II и извлеченные материалы Wargus используются как
справочник и как временные runtime placeholders. Оригинальный Warcraft II файл
может лежать в `content/assets/` только как `original_placeholder` с записью в
`content/assets/placeholder_manifest.json` и планом замены.

## Источники референсов

| Источник | Для чего использовать | Что нельзя делать |
| --- | --- | --- |
| Локальная Warcraft II BNE | Screenshots UI, юнитов, зданий, terrain, briefing, эффекты, аудио-поведение; временные runtime placeholders. | Использовать оригинальные файлы как финальные ассеты проекта или добавлять их без manifest entry. |
| Локальный Wargus | Имена, states, animation directions, button mappings, script metadata. | Смешивать reference-анализ с финальными runtime-ассетами без manifest/catalog. |
...
Временные оригинальные runtime placeholders хранятся в тех же runtime-папках, что
и будущие новые ассеты, но отличаются manifest metadata:

```json
{
  "asset_id": "alliance_units_atlas",
  "runtime_path": "res://content/assets/textures/units/alliance_units_atlas.png",
  "origin": "original_placeholder",
  "status": "original_placeholder",
  "replacement_target": "new_alliance_units_atlas"
}
```
...
- Нельзя добавлять оригинальные Warcraft II assets как финальный runtime content.
  Они допускаются только как `original_placeholder` с manifest entry.
...
- original/reference-only материалы не попали в runtime папки без статуса
  `original_placeholder`;
- каждый `original_placeholder` имеет replacement target и owner;
